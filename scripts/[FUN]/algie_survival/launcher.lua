...
function main()
	...
	while true do
		...
		while gBlip ~= -1 do
			if gForce or (PlayerIsInAreaXYZ(x,y,z,1) and PedMePlaying(gPlayer,"DEFAULT_KEY",true)) then -- DEFAULT_KEY basically means the ped isn't "doing" anything
				if gForce or IsButtonBeingPressed(9,0) then
					BlipRemove(gBlip)
					gBlip = -1
					play_script()
					gForce = false
...
-- command to make the survival start ASAP
SetCommand("start_algie_survival",function()
	gForce = true
end)
